(function() {
    ...
    var pageInterface = {
                ...
                Utils.request(null, {
                    ...
                    success: function(res) {
                        if (res.status == 'success') {
                            _this.tradeAuto = _auto;
                            Utils.createAlert('设置成功');
                            _this.checkTradeAuto();
                        }
                    }
                });
            });
        ...
        checkTradeAuto: function() {
            setInterval(function() {
                _this.isReachAutoTrade();
            }, Utils.config.trade_speed);
        },
        isReachAutoTrade: function() {
            var _this = this;
            Utils.request(null, {
                type: 'post',
                url: Utils.config.apiHost + Utils.config.threshold_trgger,
                dataType: 'json',
                contentType: 'application/json',
                data: {},
                success: function(res) {
    ...
    $(document).ready(function() {
        ...
        $("#js-media").on("click", function() {
            ...
            audio.currentTime = 0;
            if ($(this).hasClass('icon-nomusic')) {
                $(this).removeClass('icon-nomusic');
                audio.play();
            } else {
                $(this).addClass('icon-nomusic');
                audio.pause();
            }
        });

        $('#musicTips-close').on('click', function() {
            $('#musicTips').removeClass('mt-show');
            $("#js-media").click();
        });
    });
})();